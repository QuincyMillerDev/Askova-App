                    ...
                    {(isHovering || isCopied) &&
                        ...
                        message.status !== "streaming" && (
                            <Button
                                ...
                                onClick={copyToClipboard} // No change needed here, just calls the async function
                        ...
                        components={{
                            ...
                            code: ({ node: _node, ...props }) => {
                                ...
                                return codeProps.inline ? (
                                    <code
                                        className="px-1 py-0.5 rounded bg-muted/50 text-sm font-mono"
                                        {...props}
                                    />
                                ) : (
                                    <code
                                        className="block p-2 rounded bg-muted/50 text-sm font-mono my-2 overflow-x-auto"
                                        {...props}
                                    />
                                );
                            },
...
function StreamingContent({ content }: { content: string }) {
    return (
        <div className="relative">
            ...
            <span className="inline-block h-4 w-2 ml-0.5 align-middle">
        <span className="animate-pulse-fast block h-4 w-[2px] rounded-full bg-current opacity-75"></span>
      </span>
        </div>
    );
}
